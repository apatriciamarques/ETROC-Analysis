...
def df_apply_fit_distance_cut(
    ...
    import scipy.stats
    ###### MAJOR DIFFERENCE FROM THE DIAG DIST FIT
    fit = scipy.stats.linregress(x=data_df[column_1].astype(float), y=data_df[column_2].astype(float))

    ### PRINT ITS PARAMETERS
    print("\n\nColuna 1"), print(column_1)
    print("Coluna 2"), print(column_2)
    print("Fit Slope:"), print(fit.slope)
    print("Fit Intercept:"), print(fit.intercept)
...
def df_apply_corner_cut(
    ...
    cut_direction:str,
    column_1:tuple, #x axis (horizontal) plots after time cuts
    column_2:tuple, #y axis (vertical) plots after time cuts
    edge_1:str,
    edge_2:str,
    radius:str,
    keep_nan:bool=False,
...
def df_apply_time_cut_governor(
    ...
    #REMEMBER ITS THE OPPOSITE logic if you're looking at 'plot_after_time_cuts'
