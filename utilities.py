...
def make_boards_toa_correlation_plot(
    ...
    x = (data_df["time_of_arrival_ns_{}".format(board_a)]).tolist()
    y = (data_df["time_of_arrival_ns_{}".format(board_b)] - data_df["time_of_arrival_ns_{}".format(board_a)]).tolist()
    ...
    fig = px.scatter(
        ...
        y=y,
        labels = {
            str("time_of_arrival_ns_{}".format(board_a)): "Board {} Time of Arrival [ns]".format(board_a),
            str("time_of_arrival_ns_{}_minus_{}".format(board_b, board_a)): "Time Resolution [ns] (Board {} - Board {})".format(board_b, board_a),
        },
        title = "Time of Arrival in ns board {} minus board {}<br><sup>Run: {}{}</sup>".format(board_a, board_b, run_name, extra_title),
        opacity = 0.1,
        trendline="ols",
        range_x=range_x,
        range_y=range_y,
    ...
    fig.write_html(
        base_path/'toa_board{}_minus_board{}_scatter.html'.format(board_a, board_b),
        full_html = full_html,
        include_plotlyjs = 'cdn',
    )
    ...
    fig.write_html(
        base_path/'toa_board{}_minus_board{}.html'.format(board_a, board_b),
        full_html = full_html,
        include_plotlyjs = 'cdn',
    )
...
def make_toa_correlation_plots(
    ...
    # Patrícia created this
    make_boards_toa_correlation_plot(
        board_a=board_ids[2], #1
        board_b=board_ids[1], #3
        data_df=data_df,
        base_path=base_path,
        run_name=run_name,
        extra_title=extra_title,
